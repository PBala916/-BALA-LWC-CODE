...
export default class PdfGenerationForContacts extends LightningElement {
    @api recordId
    ...
    pdfHandler(){
        ...
        generatePDF({ recordId:this.recordId, htmlData:content.outerHTML}).then(result=>{
            ...
            window.open(`https://dream-app-1361-dev-ed.scratch.file.force.com/servlet/servlet.FileDownload?file=${result.Id}`)
        }).catch(error=>{
            console.error(error)
        })
    }
